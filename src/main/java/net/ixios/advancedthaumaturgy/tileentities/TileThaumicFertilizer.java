...
public class TileThaumicFertilizer extends TileEntity implements IAspectContainer
{
    ...
    public void updateEntity()
    {
        ...
        Material mat = worldObj.getBlock(cx, cy, cz).getMaterial();
		...
		{
			isSpecial=BotaniaCompat.isSpecialFlower(block);
		}
        if (block instanceof BlockFlower && !isSpecial)
        ...
        if (!(block== ConfigBlocks.blockManaPod))
    		return;
    ...
    private void fertilize(int x, int y, int z)
    {
    	if(!worldObj.isRemote) {
			if (herbapool > costperfertilize && ItemDye.applyBonemeal(new ItemStack(Items.dye, 1, 0), worldObj, x, y, z, FakePlayerFactory.get((WorldServer) worldObj, new com.mojang.authlib.GameProfile(UUID.fromString("9cb16868-a301-47e6-8cb2-dbc7579ce7d8"), "[advthaumPlayer]")))) {
    ...
    private void spreadSurfaceBlockFrom(int x, int y, int z)
    {
    	...
    	if (!worldObj.isAirBlock(x + fd.offsetX, y + fd.offsetY, z + fd.offsetZ))
    		return;
